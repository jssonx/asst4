...
int main(int argc, char **argv)
{
    ...
    if (thread_count <= -1)
    {
        ...
        double top_time_sequential, bottom_time_sequential;
        ...
        for (int i = 0; i < n_usage; i++)
        {
            ...
            start = CycleTimer::currentSeconds();
            bfs_top_down_sequential(g, &sol1);
            top_time_sequential = CycleTimer::currentSeconds() - start;

            start = CycleTimer::currentSeconds();
            bfs_bottom_up_sequential(g, &sol2);
            bottom_time_sequential = CycleTimer::currentSeconds() - start;
            ...
            // Test correctness of sequential topdown
            std::cout << "Testing Correctness of Sequential Top Down\n";
            for (int j = 0; j < g->num_nodes; j++)
            {
            ...
            // Test correctness of sequential bottomup
            std::cout << "Testing Correctness of Sequential Bottom Up\n";
            for (int j = 0; j < g->num_nodes; j++)
            {
                if (sol2.distances[j] != sol3.distances[j])
                {
                    fprintf(stderr, "*** Results disagree at %d: %d, %d\n", j, sol2.distances[j], sol3.distances[j]);
                    bus_check = false;
                    break;
                }
            }
            ...
            sprintf(buf, "%4d:     %8.2f     %8.2f     %8.2f     %8.2f     %8.2f\n",
                    num_threads[i], top_time, bottom_time, hybrid_time, top_time_sequential, bottom_time_sequential);
            sprintf(ref_buf, "%4d:     %8.2f     %8.2f     %8.2f\n",
                    num_threads[i], ref_top_time, ref_bottom_time, ref_hybrid_time);
